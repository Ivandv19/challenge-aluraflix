...
export const GlobalProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const videosFiltrados = videos.filter(
      (video) => video.Categoria === 'FRONT END',
    );
  ...
  useEffect(() => {
    ...
    const backendVideos = videos.filter(
      (video) => video.Categoria === 'BACK END',
    );
  ...
  const editVideo = async (id, updatedFields) => {
    try {
      const response = await fetch(
        `https://my-json-server.typicode.com/IvandevI9/api-aluraflix/Videos/${id}`,
        {
  ...
  useEffect(() => {
    if (videoSeleccionado !== null) {
      ...
      const videoSelec =
        videos.find((video) => video.id === videoSeleccionado) || {};
